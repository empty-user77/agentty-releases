...
| プラットフォーム | ファイル | 備考 |
|---|---|---|
| macOS 13+（Apple シリコン） | `Agentty-X.Y.Z-…-arm64.dmg` | 署名・公証済み |

DMG を開き、**Agentty** を**アプリケーション**にドラッグします。署名と公証がされているため、Gatekeeper の警告なしに起動します。

各リリースには `Agentty-X.Y.Z-arm64.zip` と、ダウンロードを照合できる `SHA256SUMS.txt` も含まれます。
...
Agentty は起動時と 1 時間ごとにリリースチャンネルを確認します。

macOS では署名済みの DMG をダウンロードして検証し、アプリを入れ替えて再起動します。

ダウンロードは GitHub がホストするファイルからのみ行われ、使用する前に公開されたチェックサムと照合して検証します。
...
> [!NOTE]
> 公式のリリースパイプライン外で作られたビルドは、使用統計をいっさい送りません。分析用の資格情報は公式リリースビルドにしか含まれないからです。[使用統計](/docs/telemetry)を参照してください。
