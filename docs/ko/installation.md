...
| 플랫폼 | 파일 | 비고 |
|---|---|---|
| macOS 13+ (Apple 실리콘) | `Agentty-X.Y.Z-…-arm64.dmg` | 서명 및 공증됨 |

DMG를 열고 **Agentty**를 **응용 프로그램**으로 드래그하세요. 서명과 공증이 되어 있어 Gatekeeper 경고 없이 실행됩니다.

각 릴리스에는 `Agentty-X.Y.Z-arm64.zip`과, 내려받은 파일을 대조해 볼 수 있는 `SHA256SUMS.txt`도 함께 올라갑니다.
...
Agentty는 실행 시와 매시간 릴리스 채널을 확인합니다.

macOS에서는 서명된 DMG를 내려받아 검증한 뒤 앱을 교체하고 다시 실행합니다.

내려받기는 GitHub이 호스팅하는 파일로만 이루어지며, 사용하기 전에 공개된 체크섬과 대조해 검증합니다.
...
> [!NOTE]
> 공식 릴리스 파이프라인 밖에서 만든 빌드는 사용 통계를 전혀 보내지 않습니다. 분석 자격 증명은 공식 릴리스 빌드에만 존재합니다. [사용 통계](/docs/telemetry)를 참고하세요.
